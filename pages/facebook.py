...
기존 v1.0 단일 페이지 앱의 본문 그대로 — 이 파일에는 페북광고 관련 모든 로직.
사이드바·페이지 설정·CSS·인증은 entry point(app.py)와 src/sidebar.py에서 처리.
"""
...
st.markdown(
    '<h1 style="font-size: 1.75rem; font-weight: 700; margin: 0; color: #111111;">페북 광고 상세</h1>',
...
def _build_report(ads_list, naver_agg, days, manual_overrides=None):
    rows, unmatched_naver = match_facebook_with_naver(
        ads_list, naver_agg, manual_overrides=manual_overrides
    )
    df = matched_rows_to_dataframe(rows, days=days)
    return df, unmatched_naver, rows


if generate_btn or st.session_state.get("fb_report_loaded", False):
    st.session_state["fb_report_loaded"] = True
    ...
    with st.spinner("페북 + 네이버 데이터 처리 중..."):
        ...
        current_df, unmatched_naver, rows_for_stats = _build_report(
            ads, naver_agg, int(days), manual_overrides
        )
    ...
    created_time_lookup = {
        r.ad.ad_name: getattr(r.ad, "created_time", None) for r in rows_for_stats
    }
    ...
    if unmatched_naver:
        with st.expander(
            ...
            st.markdown("**페북 광고 전체 (참고용 — URL/파라미터 비교)**")
