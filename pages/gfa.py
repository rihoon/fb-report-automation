...
if generate_btn or st.session_state.get("gfa_report_loaded", False):
    ...
    # API 데이터 없으면 안내 (전환 데이터까지 못 가져왔다는 의미)
    if not ads:
        last_err = dbg.get("errors", ["에러 없음"])[-1] if dbg.get("errors") else "에러 없음"
        if src == "mock":
            st.warning("⚠️ **시크릿 미설정** — `[naver_gfa]` 섹션 확인 필요.")
        elif src == "real_empty":
            st.info(
                f"ℹ️ **GFA API 호출 정상** (adAccountNo: `{ad_account}`) — 모든 reportTp 변형 시도했지만 데이터 0건. "
                f"GFA OpenAPI 문서 받아서 정확한 reportTp 확정 필요."
            )
        elif src == "real_error":
