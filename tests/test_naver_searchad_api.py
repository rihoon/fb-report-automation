...
2. 인증 헤더 빌드 검증
3. Mock 데이터 생성 검증
4. fetch_search_ads(use_mock=True) 동작 검증
"""
...
from src.mock_data import SearchAd, generate_search_ads
...
def test_signature_deterministic():
    """같은 입력 → 같은 서명 (재현 가능)."""
    args = ("secret", 1700000000000, "GET", "/stats")
...
def test_mock_generates_correct_count():
    ...
    ads = generate_search_ads(n_ads=10, seed=42)
    assert len(ads) == 10


def test_mock_returns_searchad_instances():
    """모든 결과가 SearchAd dataclass 인스턴스인지."""
    ads = generate_search_ads(n_ads=5, seed=42)
    for ad in ads:
        assert isinstance(ad, SearchAd)


def test_mock_data_realistic():
    """Mock 데이터가 합리적인 범위인지."""
    ads = generate_search_ads(n_ads=20, seed=42)
    for ad in ads:
...
def test_searchad_ctr_calculation():
    """CTR 계산 정확."""
    ad = SearchAd(
        campaign_name="test",
        adgroup_name="test",
...
def test_searchad_roas_calculation():
    """ROAS 계산 정확."""
    ad = SearchAd(
        campaign_name="test",
        adgroup_name="test",
...
def test_searchad_zero_impressions_ctr():
...
def test_searchad_zero_spend_roas():
...
# ────────────────────── 4. fetch_search_ads (Mock 모드) ──────────────────────

def test_fetch_search_ads_mock_mode():
    """use_mock=True 시 mock 데이터 반환."""
    end = datetime.now()
    start = end - timedelta(days=2)
    ads = fetch_search_ads(start, end, use_mock=True)
    assert isinstance(ads, list)
    assert len(ads) > 0
    assert all(isinstance(a, SearchAd) for a in ads)
...
if __name__ == "__main__":
    tests = [
        ...
        test_mock_returns_searchad_instances,
        test_mock_data_realistic,
        test_searchad_ctr_calculation,
        test_searchad_roas_calculation,
        test_searchad_zero_impressions_ctr,
        test_searchad_zero_spend_roas,
        test_fetch_search_ads_mock_mode,
    ]
    failed = []
    for t in tests:
        try:
